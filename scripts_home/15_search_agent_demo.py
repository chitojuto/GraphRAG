...
DEFAULT_TOPICS = ROOT / "data_home" / "processed" / "guide_topics.jsonl"
DEFAULT_PAGES = ROOT / "data_home" / "processed" / "pdf_pages.jsonl"
DEFAULT_ENV_FILE = ROOT / ".env"
DEFAULT_MODEL = "gpt-5.4-mini"
DEFAULT_QUERY = (
    "특별공급으로 신청했는데 가점제 점수 산정 기준과 무주택 기간 인정 범위, "
    "그리고 부적격으로 당첨 취소되는 경우는 어디에서 확인해야 하나?"
)


PLANNER_PROMPT = """당신은 청약(분양) 관련 규정 및 가이드 RAG 시스템의 search agent입니다.
사용자 질문에 답하기 위해 필요한 검색을 여러 번 나누어 수행합니다.

사용 가능한 도구:
- search_topics: LLM이 미리 추출한 쟁점/질문/판단절차 구조를 검색합니다. 적용 기준, 판단 절차, 결론 파악에 좋습니다.
- search_raw: 원문 규정/가이드 chunk를 검색합니다. 조항명, 용어 정의, 세부 문구 확인에 좋습니다.
- finish: 검색을 멈추고 최종 답변 단계로 넘어갑니다.

규칙:
- 한 번에 하나의 도구만 선택하세요.
- 이전 검색 결과와 겹치지 않게 다음 검색어를 바꾸세요.
- 서로 다른 쟁점이나 절차가 필요하면 별도 검색으로 나누세요.
...
FINAL_PROMPT = """당신은 청약(분양) 관련 규정 및 가이드를 근거로 답하는 RAG assistant입니다.
아래 검색 로그에 포함된 근거만 사용해 답하세요.
적용 기준, 판단 절차, 확인 사항, 주의사항을 구체적으로 정리하세요.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Small agentic search loop over housing-subscription topic/raw BM25 indexes")
