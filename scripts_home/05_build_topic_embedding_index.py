...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build an eTL topic embedding index with the course embedding API.")
...
def main() -> None:
    ...
    texts = [build_topic_text(record) for record in records]
    if not texts:
        raise SystemExit("no topic records found")
    ...
    print(f"topics={len(records)} dim={embeddings.shape[1]}")
    print(args.index)
    print(args.metadata)
