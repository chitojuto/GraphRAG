...
SYSTEM_PROMPT = """당신은 주택 분양/청약 안내 가이드를 근거로 답하는 RAG assistant입니다.
작업/질문 검색 결과와 원문 context가 함께 제공되면, 구조화된 작업/절차 정보를 우선 사용하고 원문은 보충 근거로만 사용하세요.
사용자의 질문에 대해 신청 경로, 자격 요건, 처리 순서, 주의사항을 구체적으로 답하세요.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="청약 topic BM25 + optional raw text context + LLM answer")
