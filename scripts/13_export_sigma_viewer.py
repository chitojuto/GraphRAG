...
DEFAULT_INPUT = ROOT / "data" / "indexes" / "issue_graph_with_similarity.json"
DEFAULT_OUTPUT = ROOT / "results" / "graph_viewer_sigma.html"
...
<head>
  ...
  <title>VAT Tribunal Issue Graph Explorer</title>
  <style>
    :root {
      color-scheme: dark;
      --bg: #101214;
...
<body>
  <aside class="left">
    <h1>VAT Tribunal Issue Graph</h1>
    <p>Search a node, then render its local neighborhood. Full graph data is loaded, but the screen draws a bounded subgraph.</p>
    ...
    <h2>Search</h2>
    <input id="search" type="search" value="사실과 다른 세금계산서" placeholder="자료상, 선의의 거래당사자, 계좌이체..." />
    <label for="typeFilter">Node type</label>
    <select id="typeFilter">
      <option value="">All types</option>
      <option>Case</option>
      <option>Issue</option>
      <option>LegalConcept</option>
      <option>FactPattern</option>
      <option>EvidenceType</option>
