...
def main() -> None:
    ...
    if not args.index.exists() or not args.metadata.exists():
        raise SystemExit(
            ...
            "Run: python scripts/05_build_issue_embedding_index.py"
        )
    ...
    for rank, idx in enumerate(top_indices, start=1):
        meta = metadata[idx]
        print(f"{rank}. score={scores[idx]:.3f} file={meta['document_id']} issue={meta['issue_index']}")
        print(f"   쟁점: {meta['issue']}")
        print(f"   결론: {meta['conclusion'][:180]}")
