...
from sct_graphrag.graph_build import add_similarity_edges, build_typed_issue_graph
from sct_graphrag.io import load_jsonl, write_json


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_INPUT = ROOT / "data" / "processed" / "issue_features.jsonl"
DEFAULT_OUTPUT = ROOT / "data" / "indexes" / "issue_graph.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build typed issue graph from extracted graph features.")
...
def main() -> None:
    args = parse_args()
    graph = build_typed_issue_graph(load_jsonl(args.input))
